"""CityBrain chat endpoints (conversation + AI decisions)."""
import uuid
import logging
...
@router.post("/message")
async def send_message(req: ChatRequest, user_id: str = Depends(current_user_id)):
    if not req.message or not req.message.strip():
        raise HTTPException(status_code=400, detail="Message cannot be empty")

    conv_id = req.conversation_id
    if conv_id:
        conv = await db.conversations.find_one({"id": conv_id, "user_id": user_id}, {"_id": 0})
        if not conv:
            conv_id = None
    if not conv_id:
    ...
    await db.messages.insert_one({
        ...
        "content": req.message, "created_at": _now(),
    })
...
async def list_conversations(user_id: str = Depends(current_user_id)):
    cur = db.conversations.find({"user_id": user_id}, {"_id": 0}).sort("updated_at", -1).limit(100)
...
async def delete_conversation(conv_id: str, user_id: str = Depends(current_user_id)):
    await db.conversations.delete_one({"id": conv_id, "user_id": user_id})
